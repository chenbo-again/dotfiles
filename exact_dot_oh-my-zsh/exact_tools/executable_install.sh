...
setup_ohmyzsh() {
  ...
  echo "${FMT_BLUE}Linking Oh My Zsh from $SOURCE_DIR...${FMT_RESET}"

  if [ -e "$ZSH" ] || [ -h "$ZSH" ]; then
    if [ "$(readlink "$ZSH" 2>/dev/null)" = "$SOURCE_DIR" ]; then
      echo "${FMT_GREEN}$ZSH already points to $SOURCE_DIR.${FMT_RESET}"
      echo
      return
    fi
    fmt_error "$ZSH already exists and is not the expected Oh My Zsh link"
    exit 1
  fi
...
main() {
  ...
  if [ -d "$ZSH" ] && [ "$(readlink "$ZSH" 2>/dev/null)" != "$SOURCE_DIR" ]; then
